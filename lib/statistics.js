...
function GetTimelineStatus(fileid, selectedLang = credentials.WorkLang) {
    var AuditTimeline = { 
        reference: GetReferenceStatus(fileid, selectedLang),
        plugins: GetPluginsStatus(fileid, selectedLang),
        preassement: GetPreAssessmentStatus(fileid, selectedLang),
        plan: GetPlanningStatus(fileid, selectedLang),
        findings: GetFindingStatus(fileid, selectedLang),
        recommendations: GetRecommendationStatus(fileid, selectedLang)
    };
...
function GetReferenceStatus(fileid, selectedLang = credentials.WorkLang) {
...
function GetPluginsStatus(fileid, selectedLang = credentials.WorkLang) {
    var pluginsStatus=0;
...
function GetFindingStatus(fileid, selectedLang = credentials.WorkLang) {
    var findingStatus=0;
...
function GetRecommendationStatus(fileid, selectedLang = credentials.WorkLang) {
...
function GetPreAssessmentStatus(fileid, selectedLang = credentials.WorkLang) {
    ...
    //writetofile = new XMLSerializer().serializeToString(data2Compare[0]);
    //doc = new Dom().parseFromString(writetofile);    

    var compare = require('dom-compare').compare;
    var result = compare(doc1, data2Compare[0]);
...
function GetPlanningStatus(fileid, selectedLang = credentials.WorkLang) {
