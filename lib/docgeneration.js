...
function LoadProcedureMatrix(fileid, selectedLang = credentials.WorkLang) {
    ...
    for (var i=0; i<vDomains.length; i++) {
        ...
        for (var j=0; j<vAreas.length; j++) {
            var vPointerArea=vAreas[j].nodeValue;
            var vArea = {
                Area: vPointerArea + '. ' + xpath.select("/Audit/ActiveITAuditDomains/Domain[@nr='" + vPointerDomain + "']/Area[@nr='" + vPointerArea + "']/title/tx[@l='" + selectedLang + "']/@name",doc)[0].nodeValue,
        ...
        for (var j=0; j<vAreas.length; j++) {
			...
			var PluginId = xpath.select("/Audit/PlugIns/PlugIn/Domain[@nr='" + vPointerDomain + "']/Area[@nr='" + vPointerArea + "']/../../@id",doc)[0].nodeValue;
            var vArea = {
                Area: vPointerArea + '. (' + PluginId + ') ' + xpath.select("/Audit/PlugIns/PlugIn/Domain[@nr='" + vPointerDomain + "']/Area[@nr='" + vPointerArea + "']/title/tx[@l='" + selectedLang + "']/@name",doc)[0].nodeValue,
                code: vPointerDomain + '.' +  vPointerArea,
                color:'white',
                rowspan:1,
                Issues: []
